...
export const CustomApiNode: React.FC<NodeProps> = ({ data, selected }) => {
  const nodeData = data as unknown as WorkflowNodeData;
  const status = nodeData.status || 'idle';
  const method = nodeData.params?.apiMethod || 'GET';
  const endpoint = nodeData.params?.apiEndpoint || 'https://api.example.com';
    ...
    <div className={`relative w-72 rounded-2xl bg-[#0f172a]/90 backdrop-blur-xl border transition-all duration-300 shadow-2xl ${
      ...
      <div className="relative flex items-center justify-between p-3.5 border-b border-white/5 bg-white/[0.02]">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-xl bg-teal-500/10 border border-teal-500/20 flex items-center justify-center text-teal-400 shadow-inner">
            <Code2 className="w-4 h-4" />
          </div>
          <div>
            <h4 className="text-xs font-semibold text-slate-100 tracking-wide">{nodeData.title || 'Custom API Call'}</h4>
            <p className="text-[10px] text-teal-300/80 font-mono">REST Webhook Endpoint</p>
          </div>
        </div>
      ...
      <div className="relative p-3.5 space-y-2 text-xs">
        <div className="flex items-center gap-2 text-[11px] font-mono bg-[#030712]/60 p-2 rounded-xl border border-white/5">
          <span className="px-1.5 py-0.5 rounded bg-teal-500/20 text-teal-300 text-[10px] uppercase font-bold">{method}</span>
          <span className="truncate text-slate-400 text-[10px]">{endpoint}</span>
        </div>

        {nodeData.executionTimeMs !== undefined && (
          <div className="flex items-center justify-between text-[10px] text-slate-400 font-mono pt-1">
            <span>Duration</span>
            <span className="text-teal-300">{(nodeData.executionTimeMs / 1000).toFixed(2)}s</span>
          </div>
        )}
      </div>
      ...
      <Handle
        ...
        className="!w-3 !h-3 !bg-teal-500 !border-2 !border-slate-900 hover:!scale-125 transition-transform"
      />
    </div>
  );
};
