...
export const ConditionNode: React.FC<NodeProps> = ({ data, selected }) => {
  ...
  const conditionType = nodeData.params?.conditionType || 'contains_text';
  const conditionVal = nodeData.params?.conditionValue || '';
  const conditionMet = nodeData.output?.conditionMet;
    ...
    <div className={`relative w-72 rounded-2xl bg-[#0f172a]/90 backdrop-blur-xl border transition-all duration-300 shadow-2xl ${
      ...
      <div className="relative flex items-center justify-between p-3.5 border-b border-white/5 bg-white/[0.02]">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-xl bg-orange-500/10 border border-orange-500/20 flex items-center justify-center text-orange-400 shadow-inner">
            <GitBranch className="w-4 h-4" />
          </div>
          <div>
            <h4 className="text-xs font-semibold text-slate-100 tracking-wide">{nodeData.title || 'Condition Logic'}</h4>
            <p className="text-[10px] text-orange-300/80 font-mono">If / Else Branch</p>
          </div>
        </div>
      ...
      <div className="relative p-3.5 space-y-2 text-xs">
        <div className="bg-[#030712]/60 rounded-xl p-2 border border-white/5 text-[11px] font-mono text-slate-300">
          Rule: <span className="text-orange-400">{conditionType}</span>
          {conditionVal && <div className="text-slate-400 truncate mt-0.5">Val: "{conditionVal}"</div>}
        </div>

        {conditionMet !== undefined && (
          <div className={`p-2 rounded-xl text-center font-semibold text-xs border ${
            conditionMet 
              ? 'bg-emerald-500/10 text-emerald-300 border-emerald-500/20' 
              : 'bg-rose-500/10 text-rose-300 border-rose-500/20'
          }`}>
            Result: {conditionMet ? 'TRUE (Pass)' : 'FALSE (Fail)'}
          </div>
        )}
        ...
        {nodeData.executionTimeMs !== undefined && (
          <div className="flex items-center justify-between text-[10px] text-slate-400 font-mono pt-1">
            <span>Duration</span>
            <span className="text-orange-300">{(nodeData.executionTimeMs / 1000).toFixed(2)}s</span>
          </div>
        )}
      </div>
      ...
      <Handle
        ...
        className="!w-3 !h-3 !bg-orange-500 !border-2 !border-slate-900 hover:!scale-125 transition-transform"
      />
    </div>
  );
};
