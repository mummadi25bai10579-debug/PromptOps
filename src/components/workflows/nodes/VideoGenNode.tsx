...
import { Video, CheckCircle2, AlertCircle, Loader2, Play } from 'lucide-react';
import { WorkflowNodeData } from '../../../types/workflow';

export const VideoGenNode: React.FC<NodeProps> = ({ data, selected }) => {
  const nodeData = data as unknown as WorkflowNodeData;
  const status = nodeData.status || 'idle';
  const animation = nodeData.params?.videoAnimation || 'Zoom In';
  const duration = nodeData.params?.videoDuration || 5;
  const videoUrl = nodeData.output?.videoUrl;
    ...
    <div className={`relative w-72 rounded-2xl bg-[#0f172a]/90 backdrop-blur-xl border transition-all duration-300 shadow-2xl ${
      ...
      <div className="relative flex items-center justify-between p-3.5 border-b border-white/5 bg-white/[0.02]">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-xl bg-pink-500/10 border border-pink-500/20 flex items-center justify-center text-pink-400 shadow-inner">
            <Video className="w-4 h-4" />
          </div>
          <div>
            <h4 className="text-xs font-semibold text-slate-100 tracking-wide">{nodeData.title || 'Video Generation'}</h4>
            <p className="text-[10px] text-pink-300/80 font-mono">FFmpeg / LTX Engine</p>
          </div>
        </div>
      ...
      <div className="relative p-3.5 space-y-2 text-xs">
        <div className="flex items-center justify-between text-[11px]">
          <span className="px-2 py-0.5 rounded-md bg-white/5 border border-white/5 text-slate-300 font-mono">{animation}</span>
          <span className="px-2 py-0.5 rounded-md bg-pink-500/10 text-pink-300 border border-pink-500/20 font-mono">{duration}s</span>
        </div>

        {videoUrl ? (
          <div className="relative rounded-xl overflow-hidden border border-pink-500/20 aspect-video bg-black">
        ...
        {nodeData.executionTimeMs !== undefined && (
          <div className="flex items-center justify-between text-[10px] text-slate-400 font-mono pt-1">
            <span>Duration</span>
            <span className="text-pink-300">{(nodeData.executionTimeMs / 1000).toFixed(2)}s</span>
          </div>
        )}
      </div>
      ...
      <Handle
        ...
        className="!w-3 !h-3 !bg-pink-500 !border-2 !border-slate-900 hover:!scale-125 transition-transform"
      />
    </div>
  );
};
